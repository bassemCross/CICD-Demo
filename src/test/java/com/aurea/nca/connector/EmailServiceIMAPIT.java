...
public class EmailServiceIMAPIT {
  ...
  @Test
  public final void getAttachmentWithAttachIMAPTest()
      throws InterruptedException, IOException, MessagingException {
    String token = IMAP + "_" + UUID.randomUUID().toString();
  ...
  @Test
  public final void getAttachmentWithoutAttachIMAPTest()
      throws InterruptedException, IOException, MessagingException {
    String token = IMAP + "_" + UUID.randomUUID().toString();
    service.send(SMTP_HOST, USER, PASSWORD, SMTP_PORT, FROM, TO, SUBJECT + token, TEXT + token,
